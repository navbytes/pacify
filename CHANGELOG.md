...
- **Import from Other Proxy Managers** — make migrating to PACify effortless
  ...
  - Also imports **PAC files via URL** and can **detect the current browser proxy** and capture it as a config
  - Import wizard with format detection, a preview + report of what will be created (and anything skipped, copyable to clipboard), merge-or-replace strategy, and an optional pre-import backup
  - Migration entry points beyond Settings: an onboarding "coming from another proxy manager?" step and an empty-state "Import existing setup" call-to-action
